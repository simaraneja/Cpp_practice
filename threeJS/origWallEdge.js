...
FoyrFP.WallEdge = function(options, geometry, material) {
    ...
    this.mesh3D = new THREE.Mesh(new THREE.BufferGeometry(), new THREE.MeshBasicMaterial({side: THREE.DoubleSide, wireframe: false}));
...
FoyrFP.WallEdge.prototype.getMesh3D = function() {
    ...
    if(this.children.length > 0)
        wall_bsp = new ThreeBSP( geometry );
    ...
    for(var i=0; i<this.children.length; i++) {
        ...
        if(aperturemesh) {
            ...
            //if(height > apertureMeshBoundingBox.min.y && height > apertureMeshBoundingBox.max.y)
            {
                ...
                if (hole) {
                    //shape.holes.push(hole);
                    var geohole = new THREE.ExtrudeGeometry(hole, {amount: this.thickness, bevelEnabled: false});
    ...
    geometry.faceVertexUvs[0].forEach(function(faceUV){
        var uv1 = faceUV[0];
        var uv2 = faceUV[1];
        var uv3 = faceUV[2];
        uv1.x = (uv1.x + offset.x)/range.x; uv1.y = (uv1.y + offset.y)/range.y;
        uv2.x = (uv2.x + offset.x)/range.x; uv2.y = (uv2.y + offset.y)/range.y;
        uv3.x = (uv3.x + offset.x)/range.x; uv3.y = (uv3.y + offset.y)/range.y;
    });
    geometry.uvsNeedUpdate = true;

    var posX = 0;
    for(var i=0; i<geometry.vertices.length; i++) {
        var vt = (posX + geometry.vertices[i].x)/width;
        if(vt < 0) {
            vt = 0;
        }
        if(vt > 1) {
        ...
        posTransformed.normalize().multiplyScalar(geometry.vertices[i].z).add(pt);

        posTransformed.sub(this.position);
        geometry.vertices[i].x = posTransformed.x;
        geometry.vertices[i].z = posTransformed.z;
    }
    ...
    if(this.material.map) {
        this.mesh3D.material.map = this.material.map;
    } else {
        this.mesh3D.material.map = texture;
    }
    this.mesh3D.material.color.setHex(this.color);
